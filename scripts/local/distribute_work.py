...
def pssh(minute='*', hour='*', day='*', cycles='1', benchmark='pgbench'):
	...
	task=\\'$_task\\'
	psshcommand='set -f && eval "$(ssh-agent -s)" && ssh-add -k ~/.ssh/git_capstone && rm -rf IaaSCloudResourceContention && git clone https://github.com/delvinuw/IaaSCloudResourceContention-paper.git && mv IaaSCloudResourceContention SCRIPT && cd ~/SCRIPT && cp /etc/crontab . && echo '$task' >> crontab && sudo mv crontab /etc/crontab && sudo chown root.root /etc/crontab && sudo service cron reload'
...
def cloneGitRepo():
	...
	git clone https://github.com/delvinuw/IaaSCloudResourceContention-paper.git && mv IaaSCloudResourceContention SCRIPT'
